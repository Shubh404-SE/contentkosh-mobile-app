...
export function ExamQuestionEditorScreen({ route, navigation }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (mode === 'create') {
        return await createExamQuestion({ businessId: businessId!, examTestId, body });
      }
      if (!questionId) throw new Error('Missing question id');
      return await updateExamQuestion({ businessId: businessId!, questionId, body });
    },
    onSuccess: async () => {
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <Text style={styles.sub}>Exam test • {examTestId}</Text>
