...
import { submitPracticeAttempt } from '../../api/practiceTestsApi';
...
type Props = NativeStackScreenProps<TestsStackParamList, 'PracticeAttempt'>;

export function PracticeAttemptScreen({ route, navigation }: Props) {
  ...
  useEffect(() => {
    const sub = navigation.addListener('beforeRemove', (e) => {
      // Let programmatic navigations (submit -> replace) proceed.
  ...
  return (
    <AttemptScreenBase
      ...
      onSubmit={async (answers) => {
        ...
        await submitPracticeAttempt({ businessId, attemptId, body });
        navigation.replace(TESTS_STACK.TEST_RESULT, { kind: 'practice', attemptId });
      }}
    />
  );
}
